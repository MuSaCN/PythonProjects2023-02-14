...
def standard_autocorrelation(symbol, lag):
    ...
    rates = rates.diff(lag).dropna() # 转为lag滞后差分
    from pandas.plotting import autocorrelation_plot
    plt.figure(figsize=(10, 5))
    autocorrelation_plot(rates)
# 这里表示EURUSD的H1收盘价的滞后50差分序列，该序列的自相关性图。
# 注意自相关性的滞后与差分滞后不要混淆了！
standard_autocorrelation(symbol='EURUSD', lag=50)
plt.show()

#%%
# hour1,hour2时段之外全部排除后的序列价格增量相关性图表
def seasonal_autocorrelation(symbol, lag=1, hour1=1, hour2=1):
    ...
    plt.figure(figsize=(10, 5))
    autocorrelation_plot(rates)
seasonal_autocorrelation("EURUSD", lag=5, hour1=1, hour2=1)
plt.show()
seasonal_autocorrelation("EURUSD", lag=25, hour1=1, hour2=1)  # （仅剩每天的第一小时）
plt.show()
'''这意味着当天的第一小时与lag天之前的第一小时的差值增量序列，具有自相关性。
    即该序列与过去紧密相关，依此类推。'''
'''注意：此处思路有问题'''
...
plt.show()
# 上面意味着，相邻时间段与lag天之前的对应时间段差值增量序列，具有自相关性。

#%%
### 所有时段的季节相关性热点图
# 该函数按照上面的思路，分别计算每个小时的lag天之间的差值序列，然后计算个小时之间的相关性
...
def hourly_signals_statistics(symbol, lag, hour, hour2, rfilter):
    ...
    H1 = rates.drop(rates.index[~rates.index.hour.isin([hour])]).reset_index(drop=True).diff(lag).dropna() # H --> hour小时lag价格的序列
    ...
    H1_shift1 = H1[1:].reset_index(drop=True)
    H2_shift1 = H2[1:].reset_index(drop=True)
    # 前一期的 previous returns for both hours
    H1_pre = H1[:-1].reset_index(drop=True)
    H2_pre = H2[:-1].reset_index(drop=True)
    ...
    predicted = H1_shift1 - (H1_pre - H2_pre) # 当前13小时的 - (前一期13小时 - 前一期14小时)
    real = H2_shift1 # 当前14小时的

    # 第一个和第二个小时的增量构建一个散点图 correlation joinplot between two series
    outcorr = pd.DataFrame()
    outcorr['Hour ' + str(hour)] = H1['close']
